...
function XepHDVPageContent() {
  ...
  async function handleSave() {
    if (!result) return;
    const toSave = result.filter((tr) => tr.doan_id != null);
    const skipped = result.filter((tr) => tr.doan_id == null).length;
    if (toSave.length === 0) { toast.error(t("Không có đoàn nào từ DB để lưu")); return; }
    try {
      await saveMut.mutateAsync(toSave.map((tr) => ({ doan_id: tr.doan_id!, hdv_id: tr.assigned_hdv_id })));
                    ...
                    <div className="border rounded-md p-2.5 space-y-2">
                      ...
                      {(() => {
                        const agentsInList = agents.filter((ag) =>
                          lockList.some((tr) => tr.agent_id === ag.id)
                        );
                      ...
                      {/* Danh sách đoàn — biến `t` ko bị shadow vì map cb đặt tên `tour` */}
              ...
              {!result ? (
                ...
                <>
                  ...
                  <div className="flex items-center gap-3 pb-1 text-xs text-muted-foreground">
                    ...
                    {result.some((tr) => tr.is_chained) && (
                      <span className="text-amber-600 flex items-center gap-0.5">
                        <Zap className="h-3 w-3" /> {result.filter((tr) => tr.is_chained).length} {t("ghép chuyến")}
                      </span>
                    )}
                    {result.some((tr) => tr._prev_hdv_id !== undefined && tr._prev_hdv_id !== tr.assigned_hdv_id) && (
                      <span className="text-amber-600 font-medium">
                        · ✱ {result.filter((tr) => tr._prev_hdv_id !== undefined && tr._prev_hdv_id !== tr.assigned_hdv_id).length} {t("thay đổi")}
                      </span>
                    )}
                  </div>
