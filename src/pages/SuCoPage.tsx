...
export default function SuCoPage() {
  ...
  const { user } = useAuth();
  const { data: logs = [], isLoading } = useDoanLogSuCo();
  ...
  const handleToggle = async (id: number, doan_id: number, is_resolved: boolean) => {
    try {
      await toggleMut.mutateAsync({ id, doan_id, loai: "su_co", is_resolved: !is_resolved });
    } catch {
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-lg font-semibold">Sự cố</h1>
