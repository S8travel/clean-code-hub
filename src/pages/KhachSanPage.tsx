...
function KhachSanPageContent() {
  ...
  const handleCreate = async () => {
    if (!newName.trim()) { toast.warning(t("Tên khách sạn bắt buộc")); return; }
    try {
      ...
      setNewDiaDiem("");
      setShowCreate(false);
      toast.success(t("Đã tạo khách sạn"));
    } catch {
      toast.error(t("Lỗi tạo khách sạn"));
    }
  };

  return (
    <div className="flex h-[calc(100vh-3rem)] overflow-hidden">
      {/* List — full-width mobile, ẩn khi đã chọn 1 KS (mobile); cạnh trái cố định ở desktop */}
      <div className={cn(
        ...
        <div className="p-3 border-b space-y-2">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-sm">{t("Khách sạn")}</h2>
        ...
        <ScrollArea className="flex-1">
          {isLoading ? (
            ...
            <p className="p-3 text-xs text-muted-foreground">{t("Không tìm thấy khách sạn")}</p>
          ) : (
            <div className="p-1">
              {filtered.map((ks) => (
                <button
      ...
      {/* Detail — ẩn trên mobile khi chưa chọn (list chiếm hết); 2 cột ở desktop */}
      <div className={cn(
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("Thêm khách sạn mới")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              <Label className="text-xs">{t("Tên khách sạn *")}</Label>
              <Input
                value={newName}
