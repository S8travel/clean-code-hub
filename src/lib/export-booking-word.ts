...
// Borders for detail cells within a 3-row group — no internal horizontal lines
...
export async function exportBookingWord(
  ...
  const totalRowCount = allEntries.length * 3; // 3 rows per entry
  ...
  // Data rows: 3 rows per entry, all entries unified
  allEntries.forEach((entry, idx) => {
    const rowAChildren: TableCell[] = [];

    // HOTEL label — horizontal, spans ALL rows (hotel + tau)
    if (idx === 0) {
      rowAChildren.push(
    ...
    if (entry.kind === "ks") {
      ...
      rowAChildren.push(cell([p(diaDiem, { size: 24 })], { width: COL_W[2], rowSpan: 3 }));
      ...
      rows.push(new TableRow({
        children: [
          cell([p(roomInfo, { bold: true, size: 28, color: "FF0000" })], { width: COL_W[3], borders: DETAIL_MIDDLE }),
        ],
      }));
      ...
      rows.push(new TableRow({
        children: [
          cell([p(bk.khach_san_website || "", { italic: true, size: 22, color: "0563C1" })], { width: COL_W[3], borders: DETAIL_BOTTOM }),
        ],
      }));

    } else {
      const { date, tau } = entry;
      const bua = tau.bua_an === "trua" ? "午餐" : "晚餐";
      const rowBText = soKhach
        ? `日遊船：${soKhach} pax`
        : (tau.set_menu_ten || "");
      const rowCText = soKhach && tau.set_menu_ten ? tau.set_menu_ten : "";
      ...
      rowAChildren.push(
        cell(
          [new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: date ? formatDateMD(date) : `Ngày ${tau.ngay_so}`, font: "Arial", size: 24 })],
          })],
          { width: COL_W[1], rowSpan: 3 }
        )
      );
      rowAChildren.push(cell([p(bua, { size: 24 })], { width: COL_W[2], rowSpan: 3 }));
      ...
      rows.push(new TableRow({
        children: [
          cell([p(rowBText, { bold: true, size: 28, color: "FF0000" })], { width: COL_W[3], borders: DETAIL_MIDDLE }),
        ],
      }));
      ...
      rows.push(new TableRow({
        children: [
          cell([p(rowCText, { italic: true, size: 22, color: "0563C1" })], { width: COL_W[3], borders: DETAIL_BOTTOM }),
        ],
      }));
    }
  });
    ...
    new TableRow({
      children: [
        cell([p("TOTAL:", { bold: true, size: 26 })], {
          width: COL_W[0] + COL_W[1], columnSpan: 2, shading: GRAY_SHADING,
        }),
        cell([p(totalText, { bold: true, size: 26, color: "FF0000" })], {
          width: COL_W[2] + COL_W[3] + COL_W[4], columnSpan: 3, shading: GRAY_SHADING,
        }),
      ],
    })
  );
