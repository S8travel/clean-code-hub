...
describe("lớp vốn chụp cả Bảo hiểm / Tip", () => {
  ...
  it("chưa gõ tay: bảo hiểm chụp mức mặc định, tip chụp null (mức tự đặt theo tuyến)", () => {
    const von = buildPhienBan(row(), ket(), now).noi_dung_von;
    expect(von.bao_hiem_moi_khach).toBe(100_000);
    // null chứ KHÔNG phải 200.000: mức tip tự đặt đổi theo tuyến của lịch trình,
    // mà lịch trình (items) cũng nằm trong bản chụp này — suy lại được.
    expect(von.tip_ngay).toBeNull();
  });

  it("OP gõ tay thì chụp đúng số đã gõ, kể cả 0", () => {
    const von = buildPhienBan(row(), ket({ bao_hiem_moi_khach: 250_000, tip_ngay: 0 }), now).noi_dung_von;
    expect(von.bao_hiem_moi_khach).toBe(250_000);
    expect(von.tip_ngay).toBe(0);
  });

  it("đổi Tip hay Bảo hiểm thì bảng so sánh NÓI RA, không để giá tụt mà không rõ vì sao", () => {
    const cu = buildPhienBan(row(), ket(), now);
    const moi = buildPhienBan(row(), ket({ tip_ngay: 3_000_000 }), now);
    const ten = soSanhPhienBan(cu, moi).thong_so.map((t) => t.ten);
    expect(ten).toContain("Tip / ngày");
  ...
  it("bản chốt TRƯỚC khi mở khoá (thiếu hẳn khoá) không đẻ ra chênh lệch giả", () => {
    // Hồi đó engine chạy đúng hằng số bảo hiểm này, nên gán mặc định là ĐÚNG dữ
    // liệu. Tip thiếu khoá đọc ra undefined, bản mới chưa gõ tay là null — cả hai
    // đều in ra "—" nên vẫn không sinh chênh lệch giả.
    const moi = buildPhienBan(row(), ket(), now);
    const cuThieuKhoa = {
      ...moi,
      noi_dung_von: { ...moi.noi_dung_von, bao_hiem_moi_khach: undefined, tip_ngay: undefined },
    } as unknown as PhienBanDeSoSanh;
    const ten = soSanhPhienBan(cuThieuKhoa, moi).thong_so.map((t) => t.ten);
    expect(ten).not.toContain("Tip / ngày");
