...
export default function KSFocEditor({
  ...
  const save = async () => {
    ...
    if (rowIds.length === 0) {
      toast.error(t("Chưa có chi phí nào để lưu FOC. Nhập giá phòng + blur trước."));
      return;
    }
    ...
    const { error } = await externalSupabase
      ...
      .in("id", rowIds);
    if (error) {
      toast.error(t("Lỗi lưu FOC") + ": " + error.message);
      return;
    }
    toast.success(t("Đã lưu FOC cho khách sạn này"));
    qc.invalidateQueries({ queryKey: ["doan_chi_phi", doanId] });
  };
  void ksId; // ksId reserved cho future filter (multi-KS update qua join)

  return (
    <span className="inline-flex items-center gap-0.5 text-xs text-muted-foreground" title={t("FOC: cứ X phòng/đêm miễn Y phòng (per tour)")}>
      FOC
      <input
        value={k}
