  ...
  return (
    <Dialog open={!!target} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-sm">Tạo đề nghị thanh toán — {target?.moTa || "Dịch vụ"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3 py-2 text-xs">
          <p>Tổng tiền: <span className="font-semibold">{fmt(target?.thanhTien ?? 0)} VND</span></p>
          <RadioGroup value={mode} onValueChange={v => onModeChange(v as "full" | "deposit")} className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="dv-full" className="text-xs cursor-pointer">
                Toàn bộ — {fmt(target?.thanhTien ?? 0)} VND
              </Label>
            </div>
            <div className="flex items-center gap-2">
          ...
          {mode === "deposit" && (
            <div className="space-y-1">
              ...
              {depositAmount > 0 && target && (
                <p className="text-[11px] text-muted-foreground">Còn lại: {fmt(target.thanhTien - depositAmount)} VND</p>
              )}
            </div>
          )}
          <div className="space-y-1">
