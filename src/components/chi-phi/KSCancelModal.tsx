      ...
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-sm">
            {target?.type === "dntt" ? t("Hủy đề nghị thanh toán") : t("Hủy sử dụng dịch vụ")}
          </DialogTitle>
        </DialogHeader>
          ...
          {target?.type === "dntt" ? (
            ...
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground">
                {t("Hủy")} {target.unpaidDnttIds.length} {t("đề nghị thanh toán đang chờ xử lý. Sau khi hủy, bạn có thể tạo đề nghị mới với số tiền chính xác hơn.")}
              </p>
              <p className="text-xs text-muted-foreground">
                {t("Các khoản đã thanh toán trước đó không bị ảnh hưởng.")}
              </p>
            </div>
          ) : (
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3 space-y-1 text-xs">
                {target && target.paidTotal > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("Đã thanh toán")} ({target.paidDnttIds.length} {t("khoản")})</span>
                    <span className="font-semibold text-destructive">{fmt(target.paidTotal)} VND</span>
                  </div>
                )}
                {target && target.unpaidDnttIds.length > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("Đề nghị chưa TT")} ({target.unpaidDnttIds.length} {t("khoản")})</span>
                    <span className="text-muted-foreground">→ {t("hủy đề nghị")}</span>
                  </div>
                )}
              </div>
              <div className="space-y-2">
                <p className="text-xs font-medium">{t("Xử lý")} {fmt(target?.paidTotal ?? 0)} VND {t("đã thanh toán")}:</p>
                <RadioGroup value={mode} onValueChange={(v) => onModeChange(v as "cong_no" | "hoan_tien")} className="space-y-2">
                  <div className="flex items-start gap-2">
                    ...
                    <Label htmlFor="ks-hoan" className="text-xs cursor-pointer">
                      ...
                      <p className="text-muted-foreground font-normal">{t("Nhà cung cấp trả lại tiền cho công ty")}</p>
                    </Label>
                  </div>
                  <div className="flex items-start gap-2">
                    ...
                    <Label htmlFor="ks-cno" className="text-xs cursor-pointer">
                      ...
                      <p className="text-muted-foreground font-normal">{t("Giữ lại làm công nợ, cấn trừ vào booking sau")}</p>
                    </Label>
                  </div>
                </RadioGroup>
              </div>
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" size="sm" className="text-xs" onClick={onSubmit} disabled={submitting}>
            {submitting ? t("Đang xử lý...") : t("Xác nhận hủy")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
