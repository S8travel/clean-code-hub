    ...
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogContent className="sm:max-w-sm">
        ...
        <DialogFooter>
          ...
          <Button size="sm" onClick={handleSave} disabled={updateMut.isPending}>Lưu</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function DoiMatKhauDialog({ open, onOpenChange }: {
  ...
  const handleSave = async () => {
    if (matKhauMoi.length < 6) { toast.warning("Mật khẩu tối thiểu 6 ký tự"); return; }
    if (matKhauMoi !== xacNhan) { toast.warning("Mật khẩu xác nhận không khớp"); return; }
    setLoading(true);
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogContent className="sm:max-w-sm">
        ...
        <DialogFooter>
          <Button variant="outline" size="sm" onClick={() => handleOpen(false)}>Hủy</Button>
          <Button size="sm" onClick={handleSave} disabled={loading}>Đổi mật khẩu</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
