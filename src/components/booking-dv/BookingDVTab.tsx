...
export default function BookingDVTab({ doanId, tenDoan, ngayDi }: Props) {
  ...
  const stats = [
    { icon: PackageSearch, labelKey: "Dịch vụ", value: total,          cls: "text-foreground" },
    { icon: Clock,         labelKey: "Chờ xác nhận", value: pendingCount,   cls: "text-amber-600" },
    { icon: CheckCircle2,  labelKey: "Đã xác nhận",  value: confirmedCount, cls: "text-emerald-600" },
    { icon: XCircle,       labelKey: "Đã hủy",        value: cancelledCount, cls: "text-muted-foreground" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <div>
          <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">{t("Dịch vụ")}</h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            {t("Tự động sync khi lưu điều tour")}
          </p>
        </div>

        {total > 0 && (
          <div className="grid grid-cols-4 gap-3">
            {stats.map(({ icon: Icon, labelKey, value, cls }) => (
              <div key={labelKey} className="rounded-lg border border-border bg-card p-3">
        ...
        {total === 0 ? (
          <div className="rounded-xl border border-dashed border-border bg-card p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">{t("Chưa có dịch vụ nào")}</p>
            <p className="text-xs text-muted-foreground/60 mt-1">
              {t("Thêm dịch vụ có phí (công ty thanh toán) vào tab Điều tour, sau đó lưu để sync về đây.")}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
