...
import { chiaLo, locBaoGia, locDoan, NGAY_GIU_CAP_NHAT_DOAN, type DoanNguon } from "./portal-sync";
...
describe("locDoan — đoàn nào lên cổng, đoàn nào bị gỡ", () => {
  ...
  const doan = (over: Partial<DoanNguon> = {}): DoanNguon => ({
    id: 1, agent_id: 2, trang_thai: "dang_chay", ngay_ve: "2026-08-15",
    portal_pushed_at: "2026-08-16T00:00:00Z", ...over,
  });
  ...
  it("đoàn về đã lâu nhưng CHƯA TỪNG đẩy → vẫn phải đẩy, không thì không bao giờ hiện", () => {
    const r = locDoan([doan({ ngay_ve: "2026-01-05", portal_pushed_at: null })], homNay);
    expect(r.hienThi).toEqual([1]);
    expect(r.canDay).toHaveLength(1);
  });
...
describe("chiaLo — cắt lô để một request quá to không giết cả lượt đẩy", () => {
  it("chia đúng kích thước, lô cuối lấy phần dư", () => {
    expect(chiaLo([1, 2, 3, 4, 5], 2)).toEqual([[1, 2], [3, 4], [5]]);
  });
  it("mảng rỗng → không có lô nào", () => {
    expect(chiaLo([], 50)).toEqual([]);
  });
  it("ít hơn một lô → đúng một lô", () => {
    expect(chiaLo([1, 2], 50)).toEqual([[1, 2]]);
  });
});
